...
public class Functions
{
    ...
    public async Task<IHttpResult> GetAsync(ILambdaContext context)
    {
        context.Logger.LogInformation("Handling the 'Get' Request");
        List<Pelis> pelis = await this.repo.GetPelisAsync();
        return HttpResults.Ok(pelis);
    }    
    [LambdaFunction]
    [RestApi(LambdaHttpMethod.Get, "/find/{actor}")]
    public async Task<IHttpResult> FindAsync(string actor, ILambdaContext context)
    {
        context.Logger.LogInformation("Handling the 'Get' Request");
        List<Pelis> pelis = await this.repo.GetPelisActoresAsync(actor);
        return HttpResults.Ok(pelis);
    }
}
